...
case "$OS" in
	Darwin*)
		...
		if [[ -z "$BUILD_TYPE" || "$BUILD_TYPE" == "build" ]]; then
			...
			# Step 10: Clone the Emscripten SDK repository
			...
			# Step 11: Navigate to the Emscripten SDK source directory
			STEP="Navigating to the Emscripten SDK source directory"
			info_msg "Step 11: $STEP"
			cd "$DEST_DIR/emsdk"

			# Step 12: Install and activate the latest Emscripten version
			STEP="Installing and activating the latest Emscripten version"
			info_msg "Step 12: $STEP"
			./emsdk install latest
			./emsdk activate latest

			# Step 13: Set up the Emscripten environment variables
			STEP="Setting up Emscripten environment variables"
			info_msg "Step 13: $STEP"
			source ./emsdk_env.sh

			# Step 14: Navigate back to Raylib and create a build directory for web targets
			...
			# Step 15: Generate Makefiles using emcmake for the web platform
			...
			# Step 16: Build Raylib for web targets using emmake
